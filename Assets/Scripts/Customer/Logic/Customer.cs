...
public class Customer : MonoBehaviour
{
    ...
    public void SetCustomer(ClientDetails clientDetails)
    {
        ...
        AnimatorOverrideController aniCO = new AnimatorOverrideController(GetComponent<Animator>().runtimeAnimatorController);
        GetComponent<Animator>().runtimeAnimatorController = aniCO;
        aniCO[aniCO.animationClips[0]] = clientDetails.clientAniClip;
    ...
    private void Move()
    {
        if (!isDragging)
        {
            ...
            {
                int stateValue = UnityEngine.Random.Range(0, 2);
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.GetType().ToString() == "UnityEngine.PolygonCollider2D")
        {
            ...
            {
                //在隨機點中隨機選一個出來
                int temp = UnityEngine.Random.Range(0, collision.transform.childCount);

                //這個隨機點和前一個隨機點不能相同，如果相同的話，就會直接出邊界
                while (temp == randomIndex)
                    temp = UnityEngine.Random.Range(0, collision.transform.childCount);
